...
export function htmlStarRating(value, options){

    return _getStarRating(value, options)

}



function _getStarRating(record, options){
    ...
    for(let i=0; i< 5; i++){
        ...
        if(v > i && v < i + 1){
                starContent += getStarHalf()
        } else if ( v > i){
                starContent += getStarFull()
        } else {
                starContent += getStarEmpty()
